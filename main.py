...
def initialize(batch_file, out_file, sid):
    global crmfile, newfile, crmDataFrame, start_id
    try:
        start_id = int(sid)
    except:
        print("Please check the start id")
        exit()
    crmfile = batch_file
    newfile = out_file
...
def DF_APPLY_FUNC(row):
    global crmfile, newfile, crmDataFrame, start_id, reachedStart
    
    if (row["ID"] != start_id) and not reachedStart:
        pass
    else:
        reachedStart = True
